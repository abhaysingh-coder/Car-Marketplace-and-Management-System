...
def Notification(request):
    try:
        if request.method == 'POST':
            ...
            NotificationModel.objects.create(Role=role, Title=title, Message=message)
        user = request.session.get('User')
        context ={
            'user' : user,
...
def Profile(request):
    try:
        user = request.session.get('User')
        # activities.objects.create(Role= user.get('Role'), Email=user.get('Email'), Activity='Admin Profile Accessed')
        context ={
            'user' : user,
            'user': request.session.get('User')
        }
...
def ApproveAdminRequest(request, role, email):
    try:
        ...
        if user:
            database_registration.objects.create(Name = user.Name, Email = user.Email, Phone_No = user.Phone_No, Password = user.Password)
            user.delete()
...
def ApproveRentRequest(request, id):
    try:
        ...
        if service:
            rent_data.loc[rent_data['Car_ID'] == service.Car_ID, 'Status'] = 'Not Available'
            service.Status = 'Approved'
...
def RemoveRentRequest(request, id):
    try:
        service = CarRent.objects.filter(Rent_ID =id)
        if service:
...
def RemoveSellingRequest(request, id):
    try:
        service = CarSelling.objects.filter(Order_ID=id).first()
        if service:
            service.Status = 'Rejected'
            service.save()
        return redirect('Admin:Selling_Request')
