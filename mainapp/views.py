...
def recommend(matches):
    ...
    available_columns = [col for col in ['Make', 'Model', 'Year', 'Engine_HP', 'Engine_Cylinders', 'Transmission_Type', 'Driven_Wheels', 'Market_Category' 'Vehicle_Size', 'Vehicle_Style', 'highway_MPG', 'city_mpg', 'Popularity', 'MSRP', 'image_url'] if col in data.columns]
...
def index(request):
    try:
        top = {} 
        for i in range(6):
            top[f'top{i + 1}'] = car_data[(car_data['Make'] + ' ' + car_data['Model']) == car_names[i]].iloc[0]
        context = {
            'cars': cars,
            'top': top,
        }
...
def collection(request):
    try:
        filters = ['Make', 'Year', 'Engine_Cylinders', 'Transmission_Type', 'Driven_Wheels', 'Vehicle_Size', 'Vehicle_Style']
        filter_data = {}
...
def detail(request, filter, detail):
    try:
        cars = car_data[car_data[filter].astype(str).str.lower() == str(detail).lower()]
        context = {
            'filter_name': filter,
            'value': detail,
...
def detail_car(request, id):
    try:
        car = car_data[car_data['Car_ID'].astype(str) == str(id)]
        if car.empty:
...
def recommandation(request):
    try:
        ...
        if request.method == 'POST':
            ...
            matches = data[(data["Make"].str.lower().str.contains(car_name.lower(), na=False)) & (data["Model"].str.lower().str.contains(car_model.lower(), na=False))]
            if matches.empty:
...
def login(request):
    try:
        if request.method == 'POST':
            ...
            if role == 'admin':
                user = Admin_Registration.objects.get(Email=email, Password=password)
                ...
                user = Customer_Registration.objects.get(Email=email, Password=password)
                request.session['User'] = {
                    'Role': 'Customer',
                    'Name': user.Name,
                    'Email': user.Email,
...
def signup(request):
    try:
        if request.method == 'POST':
            ...
            if database.objects.filter(Email=email).exists():
...
def RentDetail(request, id):
    try:
        car_df = rent_data[rent_data['Car_ID'].astype(str) == str(id)]
        if car_df.empty:
            return render(request, 'error.html', {'error': 'Car not found'})
...
def BuyNow(request, id):
    try:
        car_df = car_data[car_data['Car_ID'].astype(str) == str(id)]
        if car_df.empty:
            return render(request, 'error.html', {'error': 'Car not found'})
        car = car_df.iloc[0].to_dict()
        if request.method == 'POST':
...
def History(request):
    try:
        user = request.session.get('User')
        orders = CarOrder.objects.filter(Email=user['Email']).order_by('-Order_ID')
        order_list = []
        for order in orders:
            car = None
        ...
        sold_cars = CarSelling.objects.filter(Email=user['Email']).order_by('-Sell_ID')
        sold_list = []
        ...
        rent_cars = CarRent.objects.filter(Email=user['Email']).order_by('-Rent_ID')
        rent_list = []
        ...
        prediction_history = PredictionHistory.objects.filter(Role=user['Role'], Email=user['Email']).order_by('-Predict_ID')
        recommendation_history = RecommandationHistory.objects.filter(Role=user['Role'], Email=user['Email']).order_by('-Recommandation_ID')
        context = {
            'orders': order_list,
            'sold_cars': sold_list,
...
def OrderDetail(request, type, id):
    try:
        ...
        order_list = []
        if type == 'Order':
            orders = CarOrder.objects.filter(Email=user['Email'], Order_ID=id).first()
            if orders is None:
            ...
            orders = CarSelling.objects.filter(Email=user['Email'], Sell_ID=id).first()
            if orders is None:
            ...
            orders = CarRent.objects.filter(Email=user['Email'], Rent_ID=id).first()
            if orders is None:
        ...
        car_df = select_data[select_data['Car_ID'].astype(str) == str(orders.Car_ID)]
        if car_df.empty:
            return render(request, 'error.html', {'error': 'Car data not found'})
        car = car_df.iloc[0].to_dict()
        order_list.append({'order': orders, 'car': car, 'type': type, 'Status': orders.Status})
        context = {
    ...
    try:
        if request.method == 'POST':
            ...
            if len(selling_data) == 0:
                ...
                id = selling_data['Car_ID'].iloc[-1] + 1
            sample = pd.DataFrame([{
                'Car_ID' : id,
                'Make': request.POST.get('Make'),
                'Model': request.POST.get('Model'),
            ...
            path = os.path.join(BASE_DIR, 'Data', 'Selling Data.csv')
            selling_data.to_csv(path, index=False)
...
def RecommandationDetail (request, id):
    try:
        user = request.session.get('User')
        recommendation = RecommandationHistory.objects.filter(Recommandation_ID=id, Role=user['Role'], Email=user['Email']).first()
...
def PredictionDetail(request, id):
    try:
        user = request.session.get('User')
        prediction = PredictionHistory.objects.filter(Predict_ID=id, Role=user['Role'], Email=user['Email']).first()
        if prediction is None:
